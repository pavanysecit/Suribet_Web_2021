...
public class Web_Skinfiri_CardLogin_RemoveBets_AccountDeatilsPageCancelBet_BalanceRefresh_Validation extends Web_Skinfiri_URL_OnlineLogin {
	WebDriver driver;
	
	public Web_Skinfiri_CardLogin_RemoveBets_AccountDeatilsPageCancelBet_BalanceRefresh_Validation() throws Exception {
...
//		String alterStr=new StringBuilder(Ddetails).replace(10,19,"").toString();
		System.out.println("New Draw number format: "+ Ddetails);
...
//		String alterDraw = alterStr.replaceAll("[^0-9]","");
		...
		Pick7.click();
		Thread.sleep(1000);
		System.out.println("Bets selected on the Betting table");
		...
		WebElement stakerows = driver.findElement(By.xpath("//div[@class='skinfri_stake_bwt_w']/div[3]/span[2]")); 
		...
		WebElement remove = driver.findElement(By.xpath("//div[@ng-click='DailyGameCtrl.removeSlip(slips)']")); 
		remove.click();
		Thread.sleep(2000);
		WebElement remove1 = driver.findElement(By.xpath("//div[@ng-click='DailyGameCtrl.removeSlip(slips)']")); 
		remove1.click();
		Thread.sleep(2000);
...
//		WebElement back = driver.findElement(By.xpath("//div[@ng-click='globalrouletteCtlr.ClearFields()']"));
		...
		WebElement slipdate = driver.findElement(By.xpath("//ul[@class ='ul_clear slipgrid_d slipgrid_4']/li[2]"));
		System.out.println("Draw from from the Cancel Slip details: "+slipdate.getText());
		String alterDrawSlip = slipdate.getText().substring(0, 11);
		//String alterDrawSlip = slipdate.getText().replaceAll("[^0-9]","");
		Assert.assertEquals(Ddetails, alterDrawSlip);
	...
	public void web_Login_to_suribet_website_with_valid_login_details_Click_on_Skinfiri_module_link_change_to_card_mode_of_payment() throws Throwable {
	...
	@Then("^Web: Cancel the placed bet and accept all the alerts with validation messages and verify the amount being deposited and refreshed after cancelling the bets in Skinfiri module$")
	public void web_Cancel_the_placed_bet_and_accept_all_the_alerts_with_validation_messages_and_verify_the_amount_being_deposited_and_refreshed_after_cancelling_the_bets_in_Skinfiri_module() throws Throwable {
	    Thread.sleep(2000);
	    driver.quit();
	}
}

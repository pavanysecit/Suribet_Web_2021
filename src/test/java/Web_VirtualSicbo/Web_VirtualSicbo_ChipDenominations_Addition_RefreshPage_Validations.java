...
public class Web_VirtualSicbo_ChipDenominations_Addition_RefreshPage_Validations  extends Web_VirtualSicbo_URL_OnlineLogin {
	WebDriver driver;
	
	public Web_VirtualSicbo_ChipDenominations_Addition_RefreshPage_Validations() throws Exception {
	...
	public void web_Chrome_browser_suribet_website_valid_URL_virtual_roulette_module_place_bet_chip_denominations_lock_and_unlock_state_of_chip_denominations_refresh_page_login_and_logout() throws Throwable {
	    
	}

	@When("^Web: Login to suribet website with valid login details, select VirtualSicbo module link$")
	public void web_Login_to_suribet_website_with_valid_login_details_select_VirtualSicbo_module_link() throws Throwable {
...
//		driver.
		...
		String Drawdetail;
		if(Ddetails.length()>=17) {
			Drawdetail = Ddetails.substring(0, 10);
		}else
		{
			Drawdetail = Ddetails.substring(0, 9);
		}
		...
		System.out.println("Chips are in lock with roulette table numbers and verified after again relogin condition");
		Thread.sleep(2000);
	...
	@Then("^Web: After placed bet check for the chips are still in lock condition for selected bets later navigate to other tabs and refresh virtual roulette game page, logout and again login with same credentails$")
	public void web_After_placed_bet_check_for_the_chips_are_still_in_lock_condition_for_selected_bets_later_navigate_to_other_tabs_and_refresh_virtual_roulette_game_page_logout_and_again_login_with_same_credentails() throws Throwable {
	   
	}

	@Then("^Web: Redirect to same draw num and verify whether previously placed bet are still in lock with selected numbers on roulette table$")
	public void web_Redirect_to_same_draw_num_and_verify_whether_previously_placed_bet_are_still_in_lock_with_selected_numbers_on_roulette_table() throws Throwable {
	   Thread.sleep(2000);
	   driver.quit();
	}
}

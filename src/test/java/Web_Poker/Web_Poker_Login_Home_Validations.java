...
public class Web_Poker_Login_Home_Validations {
	WebDriver driver;
	...
	public void web_Open_the_chrome_browser_Enter_the_valid_suribet_URL_Click_on_Vitual_Roulette_module_link_enter_valid_and_invalid_login_details_and_click_on_home_button() throws Throwable {
		...
		/*
		 * For Validation lets launch the poker game and verify the account is not visible and validate the logout state in the roullete game for invalid login credentials
		 */
		Actions actions = new Actions(driver);
		actions.moveToElement(VirtaulGames1).perform();
		Thread.sleep(2000);

		// Selecting the VirtualPoker game from the dropdown
		WebElement subMenuOption = driver.findElement(By.xpath("(//a[(@href= '/betting/VirtualRoulette/')])[2]")); 
		System.out.println("Dropdown visible: "+subMenuOption.isDisplayed());

		//Mouse hover menuOption 'VirtualPoker'
		actions.moveToElement(subMenuOption).click().build().perform();
		Thread.sleep(2000);
		System.out.println("Done Mouse hover and selecting 'VIRTUAL ROULETTE' from Menu");
		...
		Thread.sleep(2000);
		WebElement subMenuOption1 = driver.findElement(By.xpath("(//a[(@href= '/betting/VirtualRoulette/')])[2]")); 
		actions.moveToElement(subMenuOption1).click().build().perform();
		Thread.sleep(10000);
		System.out.println("Virtual Roulette is selected from the dropdowns");
	...
	@Then("^Web: User should able to see the virtual poker module link in the home page and should see virtualpoker module home page after login & clicking on virtualpoker module$")
	public void web_User_should_able_to_see_the_virtual_poker_module_link_in_the_home_page_and_should_see_virtualpoker_module_home_page_after_login_clicking_on_virtualpoker_module() throws Throwable {

...
public class Web_VSicbo_PlaceBet_Online_CancelSlip_AllUsecases extends Web_VSicbo_URL_Login{
private static WebDriver driver;
			
	public  Web_VSicbo_PlaceBet_Online_CancelSlip_AllUsecases() throws InterruptedException, MalformedURLException {
	...
	public void mobile_Chrome_browser_suribet_website_valid_URL_login_details_VS_module_current_draw_bet_numbers_Forward_Confirn_ClearAll_Cancel_Slip_button_Balance_and_confirmation_message_on_cancel() throws Throwable {
		driver.findElement(By.cssSelector(".fa.fa-times-circle-o")).click();
		Thread.sleep(2000);
	}

	@When("^Mobile: Login to suribet website with valid login details, Click on Virtual Sicbo module link, select the bet numbers, place the bet, cancel the bet within (\\d+) min, again place a bet and check the visibility of cancel bet$")
	public void mobile_Login_to_suribet_website_with_valid_login_details_Click_on_Virtual_Sicbo_module_link_select_the_bet_numbers_place_the_bet_cancel_the_bet_within_min_again_place_a_bet_and_check_the_visibility_of_cancel_bet(int arg1) throws Throwable {
		//Storing the balance before placing the bet
		String Mbalance = driver.findElement(By.id("spnUserBalance")).getText();
		System.out.println("Balance of the account is before placing the bet: " +Mbalance);
		...
		System.out.println("Current balance of the account after cancelling the slip is: " +Cbalance);
...
//		executor.executeScript("arguments[0].click();", cls);
		...
		//Verifying the recently placed bet in account summary tab section which should not be clickable
		...
		WebElement NewDrawNo2 = driver.findElement(By.xpath(dd));
		NewDrawNo2.click();
		Thread.sleep(4000);
	...
	public void mobile_Bet_should_be_placed_successfully_from_online_account_and_Canncel_slip_button_should_be_displayed_in_VS() throws Throwable {

	}

	@Then("^Mobile: Clicking on Cancel before (\\d+) min, bet should get cancelled and confirmation message should be displayed in VS$")
	public void mobile_Clicking_on_Cancel_before_min_bet_should_get_cancelled_and_confirmation_message_should_be_displayed_in_VS(int arg1) throws Throwable {
	    
	}

	@Then("^Mobile: Balance should get refunded to online account if the bet is cancelled within (\\d+) min in VS$")
	public void mobile_Balance_should_get_refunded_to_online_account_if_the_bet_is_cancelled_within_min_in_VS(int arg1) throws Throwable {
	    
	}

	@Then("^Mobile: Cancel slip button should get disappeared from the betting slip after (\\d+)min and user should not be able to cancel the bet in VS$")
	public void mobile_Cancel_slip_button_should_get_disappeared_from_the_betting_slip_after_min_and_user_should_not_be_able_to_cancel_the_bet_in_VS(int arg1) throws Throwable {
	    
	}

	@Then("^Mobile: Validation message should get displayed when user clicked on Cancel Slip button and then try to click on Cancel Slip button after (\\d+) min in PopUp window and amount should not get refunded in VS$")
	public void mobile_Validation_message_should_get_displayed_when_user_clicked_on_Cancel_Slip_button_and_then_try_to_click_on_Cancel_Slip_button_after_min_in_PopUp_window_and_amount_should_not_get_refunded_in_VS(int arg1) throws Throwable {
	    
	}

	@Then("^Mobile:  Cancel slip button should be displayed in Account details of the Online and Slip should get cancelled successfully within (\\d+) minute in VS$")
	public void mobile_Cancel_slip_button_should_be_displayed_in_Account_details_of_the_Online_and_Slip_should_get_cancelled_successfully_within_minute_in_VS(int arg1) throws Throwable {
	   
	}

	@Then("^Mobile: Cancel slip button should get disappeared in Account details of the Online account after (\\d+) min in VS$")
	public void mobile_Cancel_slip_button_should_get_disappeared_in_Account_details_of_the_Online_account_after_min_in_VS(int arg1) throws Throwable {
	    Thread.sleep(2000);
	    driver.quit();
	}
}

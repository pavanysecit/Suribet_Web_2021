...
public class Web_LiveSpin2Win_Search_DrawResult_Validations extends Web_LiveSpin2Win_URL_OnlineLogin {
WebDriver driver;
	
	public Web_LiveSpin2Win_Search_DrawResult_Validations() throws Exception {
		driver =Web_LiveSpin2Win_URL_OnlineLogin();
	}
	
	@Given("^Web: Chrome browser, suribet website valid URL, live Spin(\\d+)Win module, click on search draw tab and verify the previous draw results are displayed to the user$")
	public void web_Chrome_browser_suribet_website_valid_URL_live_Spin_Win_module_click_on_search_draw_tab_and_verify_the_previous_draw_results_are_displayed_to_the_user(int arg1) throws Throwable {
	...
	public void web_Change_the_dates_and_verify_under_search_is_selected_all_draw_results_are_displayed_to_user_in_draw_result_tabs_with_and_without_win_combinations() throws Throwable {
		...
		WebElement DrawResult = driver.findElement(By.xpath(" //*[text()=' Tropicana Draw Results']"));
		Assert.assertEquals(" Tropicana Draw Results", DrawResult.getText());
		...
		WebElement search = driver.findElement(By.xpath("//*[@ng-click='grLiveCtlr.GetFilterResults()']")); 
		search.click();
		Thread.sleep(2000);
		...
		WebElement cdraw = driver.findElement(By.xpath("//*[@class='color-primary ng-binding']")); 
		System.out.println("Current draw num: "+cdraw.getText());
		int i=Integer.parseInt(cdraw.getText());  

		//previous draw results 
		WebElement previousdraw = driver.findElement(By.xpath("//*[@class='color-secondary ng-binding']")); 
		...
		WebElement color = driver.findElement(By.xpath("//*[@class='result-number text-white px-3 py-2 rounded h4 mb-1 ']")); 
		String drawcolor = color.getAttribute("style");
		if(drawcolor.equalsIgnoreCase("background-color: rgb(218, 0, 0);"))
		{
			Boolean red =drawcolor.equalsIgnoreCase("background-color: rgb(218, 0, 0);");
			System.out.println("Draw result "+presult+" color is red: "+red );
			
		}else if(drawcolor.equalsIgnoreCase("background-color: rgb(0, 0, 0);")){
			Boolean black =drawcolor.equalsIgnoreCase("background-color: rgb(0, 0, 0);");
			System.out.println("Draw result "+presult+" color is black: "+black);
		}else {
			Boolean green =drawcolor.equalsIgnoreCase("background-color: rgb(0, 153, 72);");
			System.out.println("Draw result "+presult+" color is green: "+green);
		}
	
//		//Verify the number of previous results are displayed to the user
//		int firstdraw = i1;
//		System.out.println(firstdraw);
//		WebElement lastdrawresult = driver.findElement(By.xpath("//li[@ng-repeat ='res in globalrouletteCtlr.VrResults track by $index']["+firstdraw+"]/div/div/span[2]"));
...
//		System.out.println("\n");
		...
		search.click();
		Thread.sleep(2000);
		WebElement searchresult = driver.findElement(By.xpath("//*[@ng-click='grLiveCtlr.lfAccordion($index)']/div[@class='ng-binding']")); 
		Assert.assertTrue(searchresult.getText().contains("Draw: #1")); 
		...
		WebElement input = driver.findElement(By.xpath("//input[@ng-model='globalrouletteCtlr.FilterDrawNum']"));
		if(r==1) {
			input.click();
			Thread.sleep(2000);
			...
			search.click();
			Thread.sleep(2000);
			WebElement validresult = driver.findElement(By.xpath("//li[@ng-repeat ='res in globalrouletteCtlr.VrResults track by $index'][1]/div/div/span[2]"));
			Assert.assertEquals(String.valueOf(r), validresult.getText());
			Assert.assertTrue(validresult.isDisplayed());
			WebElement validdate = driver.findElement(By.xpath("//li[@ng-repeat ='res in globalrouletteCtlr.VrResults track by $index'][1]/div/h6"));
		...
		WebElement waittime = driver.findElement(By.xpath("//span[@ng-hide='globalrouletteCtlr.DrawIdReceivedNoMoreBet==globalrouletteCtlr.ActiveDrawId']")); 

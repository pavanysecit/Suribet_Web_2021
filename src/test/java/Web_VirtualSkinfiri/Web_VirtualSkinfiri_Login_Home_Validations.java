...
public class Web_VirtualSkinfiri_Login_Home_Validations {
	WebDriver driver;
	...
	public void web_Open_the_chrome_browser_Enter_the_valid_suribet_URL_Click_on_VirtualSkinfiri_module_link_enter_valid_and_invalid_login_details_and_click_on_home_button() throws Throwable {
		...
		/*
		 * For Validation lets launch the Virtual Skinfiri game and verify the account is not visiable and validate the logout state in the roullete game for invalid login credentials
		 */
		...
		// Selecting the VirtualSkinfiri game from the dropdown
		WebElement subMenuOption = driver.findElement(By.xpath("(//a[(@href= '/Betting/VirtualDailygame/')])[2]")); 
		System.out.println("Dropdown visible: "+subMenuOption.isDisplayed());
	
		//Mouse hover menuOption 'Virtual Skinfiri'
		actions.moveToElement(subMenuOption).click().build().perform();
		Thread.sleep(2000);
		System.out.println("Done Mouse hover and selecting 'Virtual Skinfiri' from Menu");

		/*
		 * Landed to the Virtual Skinfiri game page
		...
		System.out.println("After Virtual Skinfiri game loaded, account state is in logoff condition and verified: "+ UID.isDisplayed());
		...
		//Login with valid credentials and verify the state of the virtualskinfiri Game
		login1.click();
		Thread.sleep(2000);
		...
		Thread.sleep(2000);
		WebElement subMenuOption1 = driver.findElement(By.xpath("(//a[(@href= '/Betting/VirtualDailygame/')])[2]")); 
		actions.moveToElement(subMenuOption1).click().build().perform();
		Thread.sleep(10000);
		System.out.println("Virtual Skinfiri is selected from the dropdowns");
	...
	@Then("^Web: User should able to see the VirtualSkinfiri module link in the home page and should see VirtualSkinfiri module home page after login & clicking on VirtualSkinfiri module$")
	public void web_User_should_able_to_see_the_VirtualSkinfiri_module_link_in_the_home_page_and_should_see_VirtualSkinfiri_module_home_page_after_login_clicking_on_VirtualSkinfiri_module() throws Throwable {
